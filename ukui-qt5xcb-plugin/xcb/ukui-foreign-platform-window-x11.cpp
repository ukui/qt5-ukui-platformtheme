...
#include "ukui-foreign-platform-window.h"
...
ukuiForeignPlatformWindow::ukuiForeignPlatformWindow(QWindow *window, WId winId)
    : QXcbWindow(window)
{
...
ukuiForeignPlatformWindow::~ukuiForeignPlatformWindow()
{
...
QRect ukuiForeignPlatformWindow::geometry() const
{
    xcb_connection_t *conn = ukuiPlatformIntegration::xcbConnection()->xcb_connection();
    ...
    xcb_translate_coordinates_reply_t *translateReply =
        xcb_translate_coordinates_reply(conn, xcb_translate_coordinates(conn, m_window, ukuiPlatformIntegration::xcbConnection()->rootWindow(), 0, 0), 0);
    if (!translateReply) {
        free(geomReply);
        return QRect();
    }
...
QMargins ukuiForeignPlatformWindow::frameMargins() const
{
...
#ifdef Q_OS_LINUX
void ukuiForeignPlatformWindow::handleConfigureNotifyEvent(const xcb_configure_notify_event_t *event)
{
...
void ukuiForeignPlatformWindow::handlePropertyNotifyEvent(const xcb_property_notify_event_t *event)
{
...
QNativeWindow *ukuiForeignPlatformWindow::toWindow()
{
...
void ukuiForeignPlatformWindow::create()
{
...
void ukuiForeignPlatformWindow::destroy()
{
    connection()->removeWindowEventListener(m_window);
}

void ukuiForeignPlatformWindow::updateTitle()
{
...
void ukuiForeignPlatformWindow::updateWmClass()
{
...
void ukuiForeignPlatformWindow::updateWindowState()
{
...
void ukuiForeignPlatformWindow::updateWindowTypes()
{
...
void ukuiForeignPlatformWindow::updateProcessId()
{
    xcb_get_property_cookie_t cookie = xcb_get_property(xcb_connection(), false, m_window,
...
void ukuiForeignPlatformWindow::init()
{
    updateTitle();
    updateWindowState();
    updateWindowTypes();
